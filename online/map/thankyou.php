<?php
function h($str) {
    return htmlspecialchars($str, ENT_QUOTES, 'UTF-8');
}
$date = (string)filter_input(INPUT_POST, 'date');
$sub = (string)filter_input(INPUT_POST, 'sub');
$tag = (string)filter_input(INPUT_POST, 'tag');
$link = (string)filter_input(INPUT_POST, 'link');
$comment = (string)filter_input(INPUT_POST, 'comment');
$photo = (string)filter_input(INPUT_POST, 'photo');

$fp = fopen('list.csv', 'a+b');
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    flock($fp, LOCK_EX);
    fputcsv($fp, [$date, $sub, $tag, $link, $comment, $photo,]);
    rewind($fp);
}

flock($fp, LOCK_SH);
while ($row = fgetcsv($fp)) {
    $rows[] = $row;
}
flock($fp, LOCK_UN);
fclose($fp);

?>
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8"/>
<meta name="viewport" content="width=device-width">
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.4.1/jquery.min.js"></script>
<script src="ramdom.js"></script>
<title>オススメを集める | 大 chotto crazy 2020</title>
<link rel="stylesheet" type="text/css" href="/css/programs.css" />
<style type="text/css">
.about_map #sub,
.about_map #links h2 {color:orange;}
.about_map u,
.about_map #links a {background:orange;}
.about_map #links a:hover {background:#eee; color:orange;}
.about_map #more_text {border-bottom:0.2rem solid orange;}
.about_map #more_text h2 {
	color:#fff;
  -webkit-text-stroke: 0.05rem orange;
  text-stroke: 0.05rem orange;
}
</style>
<script type="text/javascript">
$(function(){
$("#").load("__");
$("#").load("__");
})
</script>
</head>
<body>
<div id="introduce">
<div id="more" class="about_map">
<p><u>オススメを集める</u><br/><b id="sub">about coding</b><br/>
<span id="date">2020.12.15</span></p>
<div id="more_text">
<h2>ウェブサイト[岡山県オススメ60スポット]の基となったオススメを集めるPHPコード β ver. の配布を開始します。</h2>
<p>投稿は、古い情報から順に表示され、新しい投稿は一番下に追加される仕様です。<br/>地図情報をまとめる以外にも、時系列に何かを整理／管理するためにこのウェブコードを応用することができるかと思います。</p>
<p>特に真新しい仕掛けはありません。<br/>ウェブサイト作り初心者でも簡単に仕様することができるシンプルな構成のウェブページです。</p>
<p>下記のリンクから、トップページ [index.php] ／投稿ページ [about.php] ／ CSS [map.css] をパッケージした zipファイルをダンロードできます。</p>
<p>オススメなどを整理／管理するウェブサイトの制作に、ぜひご使用ください。</p>
</div>
<div id="links">
<h2>Sample
<a href="/online/map/" target="_parent">/online/map/</a>
</h2>
<h2>Download
<a href="/online/map/download.zip" target="_parent">/online/map/download.zip</a>
</h2>
<p>使用例
<a href="/online/map/" target="_blank" rel="noopener noreferrer">オススメを集める β ver.</a>
<a href="http://vg.pe.hu/publication/value/online/" target="_blank" rel="noopener noreferrer">大切にすることを大切にするためのオンライン展</a>
</p>
<p>応用編
<a href="/ccm/" target="_blank" rel="noopener noreferrer">ちょっとクレイジーな仲間たち</a>
</p>
</div>
</div>
</div>
</body>
</html>
